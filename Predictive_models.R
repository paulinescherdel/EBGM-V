...
# -----------------------------
  ...
  correl <- function(training_data, val){
    g<-ggcorrplot(cor(training_data[val], method="spearman"),         
                  type = "upper", show.diag = T, title = "", legend.title = "", 
                  colors = c("darkred", "green", "darkred"),
                  tl.srt = 0, tl.col = "black", tl.cex = 7, 
                  lab=T, lab_size = 3.0)
    return(g)  
  }
  cor1_12<- correl(traindata_1_12,c("A","B", "C", "D", "E", "sexe"))
  cor1_2 <- correl(traindata_1_12,c("A","B", "C", "D", "sexe"))
  cor2_3 <- correl(traindata_1_12,c("A","B", "C", "D", "sexe"))
  cor3_5 <- correl(traindata_1_12,c("A","B", "C", "D", "sexe"))
  cor5_8 <- correl(traindata_1_12,c("A","B", "C", "D", "sexe"))
  cor8_12<- correl(traindata_1_12,c("A","B", "C", "D", "E", "sexe"))
  ...
  multicoll <- function(training_data, p1,p2,p3,p4, p5,p6){
    if (is.null(p6)==T){
      ...
      g=ggplot() +
        ...
        geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 1, ymax = 5),   fill = "green",  alpha = 0.2) +
        scale_x_discrete("") +
        scale_y_continuous(
          name = "VIF",
          limits = c(1, 8),
